#!/bin/sh
kubectl get pods -n webtor | grep Terminating | awk '{print $1}' | grep seeder | xargs kubectl delete po --force -n webtor
kubectl get pods -n webtor | grep Terminating | awk '{print $1}' | grep transcoder | xargs kubectl delete po --force -n webtor
